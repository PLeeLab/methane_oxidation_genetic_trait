...
groupInfo <- list(Ia=rownames(genomes_list)[genomes_list$group=='Ia'],
                  ...
                  NC10=rownames(genomes_list)[genomes_list$group=='NC10'])

tree_grouped <- groupOTU(object = tree_rooted, focus = groupInfo)

all_df <- data.frame(read.table(file = "3_all_dataframe.txt", header = T, stringsAsFactors = FALSE))
...
borrow_QC <- read.delim(file = "4_QC_data_IV.txt")[,c(1,4)]
borrow_QC <- unique(borrow_QC)
outgroup_sample <- data.frame(sample = "Isolated", strain="Bacteroides_ovatus_ATCC_8483")
...
pdf(file = "Fig1B.pdf", width = 7, height = 10, useDingbats=FALSE)
