...
pdf(file = "Fig4A.pdf", width = 20, height = 5, useDingbats=FALSE)
{par(mfrow=c(1,4))
# RSCU plotrix::radialplot of group Ia-pmo
sub_medians         <- colMedians(as.matrix(df_rscu_integrated_all[df_rscu_integrated_all$group=="Ia" & (df_rscu_integrated_all$gene_category == "pmo/amo"), c(23:81)]))
...
sub_medians         <- colMedians(as.matrix(df_rscu_integrated_all[df_rscu_integrated_all$group=="Ia" & (df_rscu_integrated_all$gene_category == "mxa"), c(23:81)]))
sub_medians_ordered <- sub_medians[order(names(sub_medians))]
...
sub_medians         <- colMedians(as.matrix(df_rscu_integrated_all[df_rscu_integrated_all$group=="Ia" & (df_rscu_integrated_all$gene_category == "mmo"), c(23:81)]))
sub_medians_ordered <- sub_medians[order(names(sub_medians))]
...
sub_medians         <- colMedians(as.matrix(df_rscu_integrated_all[df_rscu_integrated_all$group=="Ia" & (df_rscu_integrated_all$gene_category == "xox"), c(23:81)]))
sub_medians_ordered <- sub_medians[order(names(sub_medians))]
...
pdf(file = "Fig4AS1a.pdf", width = 10, height = 10)
heatmap.2(t(as.matrix(df_rscu_integrated_all[df_rscu_integrated_all$group=="Ia", c(23:81)])),
          main = "Type Ia",
          notecol = "black",
          density.info = "none",
          trace = "none",
...
#----------------- FIGURE 4AS1b -----------------#
gr_row        <- df_rscu_integrated_all$gene_category[df_rscu_integrated_all$group=="Ib"]
gr_row_labels <- df_rscu_integrated_all$gene_category_subunit[df_rscu_integrated_all$group=="Ib"]
col1 <- ifelse(test = gr_row=="mmo", yes = "#d9ef8b", 
               ifelse(test = gr_row=="hao", yes = "orange", 
                      ifelse(test = gr_row=="mxa", yes = "#4575b4", 
                             ifelse(test = gr_row=="xox", yes = "#b2abd2", 
                                    ifelse(test = gr_row=="pxm", yes = "#c51b8a", 
                                           ifelse(test = gr_row=="pmo/amo", yes = "#1a9850", "red"))))))
pdf(file = "Fig4AS1b.pdf", width = 10, height = 10)
heatmap.2(t(as.matrix(df_rscu_integrated_all[df_rscu_integrated_all$group=="Ib", c(23:81)])),
          main = "Type Ib",
          notecol = "black",
          density.info = "none",
          trace = "none",
...
#----------------- FIGURE 4AS1d -----------------#
gr_row        <- df_rscu_integrated_all$gene_category[df_rscu_integrated_all$group=="IIa"]
gr_row_labels <- df_rscu_integrated_all$gene_category_subunit[df_rscu_integrated_all$group=="IIa"]
col1 <- ifelse(test = gr_row=="mmo", yes = "#d9ef8b", 
               ifelse(test = gr_row=="hao", yes = "orange", 
                      ifelse(test = gr_row=="mxa", yes = "#4575b4", 
                             ifelse(test = gr_row=="xox", yes = "#b2abd2", 
                                    ifelse(test = gr_row=="pxm", yes = "#c51b8a", 
                                           ifelse(test = gr_row=="pmo/amo", yes = "#1a9850", "red"))))))
pdf(file = "Fig4AS1d.pdf", width = 10, height = 10)
heatmap.2(t(as.matrix(df_rscu_integrated_all[df_rscu_integrated_all$group=="IIa", c(23:81)])),
          main = "Type IIa",
          notecol = "black",
          density.info = "none",
          trace = "none",         
          ...
          hclustfun = function(x) hclust(x,method = 'complete'))
legend("bottom",      
       legend = c("pmo",     "mmo",     "mxa",     "hao",   "pxm"),
       col    = c("#1a9850", "#d9ef8b", "#4575b4", "orange", "#c51b8a"), 
       lty    = 1,             
       lwd    = 8,           
       cex    = 1,
       ncol   = 5)
...
#----------------- FIGURE 4AS1e -----------------#
gr_row        <- df_rscu_integrated_all$gene_category[df_rscu_integrated_all$group=="IIb"]
gr_row_labels <- df_rscu_integrated_all$gene_category_subunit[df_rscu_integrated_all$group=="IIb"]
col1 <- ifelse(test = gr_row=="mmo", yes = "#d9ef8b", 
               ifelse(test = gr_row=="hao", yes = "orange", 
                      ifelse(test = gr_row=="mxa", yes = "#4575b4", 
                             ifelse(test = gr_row=="xox", yes = "#b2abd2", 
                                    ifelse(test = gr_row=="pxm", yes = "#c51b8a", 
                                           ifelse(test = gr_row=="pmo/amo", yes = "#1a9850", "red"))))))
pdf(file = "Fig4AS1e.pdf", width = 10, height = 10)
heatmap.2(t(as.matrix(df_rscu_integrated_all[df_rscu_integrated_all$group=="IIb", c(23:81)])),
          main = "Type IIb",
          notecol = "black",
          density.info = "none",
          trace = "none",         
          ...
          hclustfun = function(x) hclust(x,method = 'complete'))
legend("bottom",      
       legend = c("pmo",     "mmo",     "mxa"),
       col    = c("#1a9850", "#d9ef8b", "#4575b4"), 
       lty    = 1,             
       lwd    = 8,           
       cex    = 1,
       ncol   = 3)
...
#----------------- FIGURE 4AS1f -----------------#
gr_row        <- df_rscu_integrated_all$gene_category[df_rscu_integrated_all$group=="III"]
gr_row_labels <- df_rscu_integrated_all$gene_category_subunit[df_rscu_integrated_all$group=="III"]
col1 <- ifelse(test = gr_row=="mmo", yes = "#d9ef8b", 
               ifelse(test = gr_row=="hao", yes = "orange", 
                      ifelse(test = gr_row=="mxa", yes = "#4575b4", 
                             ifelse(test = gr_row=="xox", yes = "#b2abd2", 
                                    ifelse(test = gr_row=="pxm", yes = "#c51b8a", 
                                           ifelse(test = gr_row=="pmo/amo", yes = "#1a9850", "red"))))))
pdf(file = "Fig4AS1f.pdf", width = 10, height = 10)
heatmap.2(t(as.matrix(df_rscu_integrated_all[df_rscu_integrated_all$group=="III", c(23:81)])),
          main = "Type III",
          notecol = "black",
          density.info = "none",
          trace = "none",         
          ...
          hclustfun = function(x) hclust(x,method = 'complete'))
legend("bottom",      
       legend = c("pmo",     "xox",     "hao"),
       col    = c("#1a9850", "#b2abd2", "orange"), 
       lty    = 1,             
       lwd    = 8,           
       cex    = 1,
       ncol   = 3)
