...
fasta_biostrings <- readDNAStringSet(filepath = "2574179704.genes.fna")
jgi_number <- gsub(x = names(fasta_biostrings), pattern = " .*", replacement = "")
jgi_codes <- c()
for (i in 1:length(fasta_biostrings)) {
  jgi_codes <- c(jgi_codes, unlist(strsplit(names(fasta_biostrings)[i], "[ ]"))[2])
}
fasta_seqinr <- read.fasta(file = "2574179704.genes.fna")
rna_counts <- read.delim(file = "GSE85736_001_methanotroph_count.txt")
...
integrated_df <- merge(x = jgi_df, y = rna_counts, by.x="jgi_codes", by.y="Gene.ID", sort=FALSE)

integrated_df_molten_basic <- melt(data = integrated_df, measure.vars = c(11:16))
colnames(integrated_df_molten_basic)[c(11,12)] <- c("experiment", "expression_level")
...
# Remove very lengthy gene with unknown function that is generating huge noise (gene: "T449DRAFT_3452")
integrated_df_molten_basic <- integrated_df_molten_basic[integrated_df_molten_basic$jgi_codes != "T449DRAFT_3452", ]

# Create a new column in the dataset to highligth the genes of the four operons "pmo", "smo","mxa", "xox" in the expression profile
integrated_df_molten_basic$operon <- ifelse(test = integrated_df_molten_basic$jgi_codes == "T449DRAFT_2848" | integrated_df_molten_basic$jgi_codes == "T449DRAFT_2846" | integrated_df_molten_basic$jgi_codes == "T449DRAFT_2847", yes = "pmo",
                                            no = ifelse(test = integrated_df_molten_basic$jgi_codes == "T449DRAFT_3928", yes = "xox",
                                                        no = ifelse(test = integrated_df_molten_basic$jgi_codes == "T449DRAFT_4152" | integrated_df_molten_basic$jgi_codes == "T449DRAFT_4155", yes = "mxa",
                                                                    no = "other")))
...
integrated_df_molten_basic_length <- integrated_df_molten_basic[integrated_df_molten_basic$experiment == "MB_31_BR1",]

p_puri_pyri_hist_genome_3132 <- ggplot(data = integrated_df_molten_basic_length)+
  ...
  theme_classic2()+
  ggtitle(label = "a", subtitle = expression(paste(italic("Methylobacter tundripaludum"), " 31/32")))+
...
p_pyri_exp_3132 <- ggplot(data = integrated_df_molten_basic, mapping = aes(x = nPyrimidines, y = (expression_level+0.5))) +
  ...
  facet_wrap(~experiment, nrow = 3, ncol = 2, scales = "free")+
  theme_classic2()+
...
integrated_df_molten <- melt(data = integrated_df, measure.vars = c(5:8))
colnames(integrated_df_molten)[c(13,14)] <- c("base", "count_genome")
integrated_df_molten <- melt(data = integrated_df_molten, measure.vars = c(7:12))
colnames(integrated_df_molten)[c(9,10)] <- c("experiment", "expression_level")
...
integrated_df_molten <- melt(data = integrated_df_molten, measure.vars = c(8,11,12))
colnames(integrated_df_molten)[c(10,11)] <- c("omic", "count")
...
# removing mxa
integrated_df_molten_no_mxa <- integrated_df_molten[integrated_df_molten$jgi_codes != "T449DRAFT_4152" & integrated_df_molten$jgi_codes != "T449DRAFT_4155", ]

# removing xox
integrated_df_molten_no_xox <- integrated_df_molten[integrated_df_molten$jgi_codes != "T449DRAFT_3928", ]

# removing pmo
integrated_df_molten_no_pmo <- integrated_df_molten[integrated_df_molten$jgi_codes != "T449DRAFT_2848" & integrated_df_molten$jgi_codes != "T449DRAFT_2846" & integrated_df_molten$jgi_codes != "T449DRAFT_2847", ]
...
for (u in 2:7) {t_casted[,u] <- t_casted[,u] / sum(t_casted[,u]) }; colSums(t_casted)
t_molten <- melt(data = t_casted, measure.vars = c(2:7))
t_molten$pmo <- "1T"
...
for (u in 2:7) {gtm_casted[,u] <- gtm_casted[,u] / sum(gtm_casted[,u]) }; colSums(gtm_casted)
gtm_molten <- melt(data = gtm_casted, measure.vars = c(2:7))
...
for (u in 2:7) {gtx_casted[,u] <- gtx_casted[,u] / sum(gtx_casted[,u]) }; colSums(gtx_casted)
gtx_molten <- melt(data = gtx_casted, measure.vars = c(2:7))
...
for (u in 2:7) {gtp_casted[,u] <- gtp_casted[,u] / sum(gtp_casted[,u]) }; colSums(gtp_casted)
gtp_molten <- melt(data = gtp_casted, measure.vars = c(2:7))
...
p_base_3132 <- ggplot(data = z_dataframe, aes(x = pmo, y = value*100, colour=base, group=base))+
  geom_point()+
  geom_line()+
...
p_gc_3132 <- ggplot(data = z_dataframe, aes(x = pmo, y = value*100, colour=gc, group=gc))+
...
p_compound_3132 <- ggplot(data = z_dataframe, aes(x = pmo, y = value*100, colour=compound, group=compound))+
...
integrated_df_molten_basic_atoms <- integrated_df_molten_basic[integrated_df_molten_basic$experiment == "MB_31_BR1",] # only 1 rep is required to calculate atom content as mRNA abundance won't be accounted for

p_atoms_NC_3132 <- ggplot(data = integrated_df_molten_basic_atoms, mapping = aes(x = Nitrogen*3, y = Carbon*3)) + # times 3 to calulate for codons
...
p_atoms_OH_3132 <- ggplot(data = integrated_df_molten_basic_atoms, mapping = aes(x = Oxygen*3, y = Hydrogen*3)) + # times 3 to calulate for codons
  geom_point(alpha  = ifelse(test = integrated_df_molten_basic_atoms$operon == "other", yes = 1, no = 0),
             size   = ifelse(test = integrated_df_molten_basic_atoms$operon == "other", yes = 2, no = 0),
             fill   = ifelse(test = integrated_df_molten_basic_atoms$operon == "pmo", yes = "purple", no = ifelse(test = integrated_df_molten_basic_atoms$operon == "mxa", yes = "green", no = ifelse(test = integrated_df_molten_basic_atoms$operon == "xox", yes = "yellow", no = "grey"))),
             colour = "grey",
...
p_Ncor <- ggplot(data = integrated_df_molten_basic_atoms, mapping = aes(x = expression_level+0.5, y = Nitrogen*3)) + # times 3 to calulate for codons
  ...
  theme(strip.background = element_blank(),
        panel.grid.major = element_blank(),
        panel.grid.minor = element_blank())+
  ggtitle(label = "i", subtitle = "MB_31_BR1")+
...
lay_matrix <- rbind(c(1, 2, NA),
                    ...
                    c(13, 13, 13),
                    c(13, 13, 13))
ggsave(filename = "0_puriPiry_3132.pdf",
       plot   = grid.arrange(p_puri_pyri_hist_genome_3132, p_pyri_exp_3132, p_base_3132, p_gc_3132, p_compound_3132, p_atoms_NC_3132, p_atoms_OH_3132, p_stats_Pyri, p_stats_C, p_stats_H, p_stats_O, p_stats_N, p_cors, layout_matrix = lay_matrix),
       device = "pdf",
       width  = 15,
       height = 27,
       useDingbats=FALSE)
...
p_atoms_OC_3132 <- ggplot(data = integrated_df_molten_basic_atoms, mapping = aes(x = Carbon*3, y = Oxygen*3)) + # times 3 to calulate for codons
  ...
  geom_point(alpha  = ifelse(test = integrated_df_molten_basic_atoms$operon == "other", yes = 0, no = 1),
             size   = ifelse(test = integrated_df_molten_basic_atoms$operon == "other", yes = 0, no = 5),
             fill   = ifelse(test = integrated_df_molten_basic_atoms$operon == "pmo", yes = "#984EA3", no = ifelse(test = integrated_df_molten_basic_atoms$operon == "mxa", yes = "#4DAF4A", no = ifelse(test = integrated_df_molten_basic_atoms$operon == "xox", yes = "#FF7F00", no = "grey"))),
             colour = "black",
             pch    = 21)+
  theme_bw()+
  ...
  ylab(label = "Oxygen atoms per codon")+
  NULL
ggsave(filename = "0_countergradient_3132.pdf",
       plot = p_atoms_OC_3132,
       device = "pdf",
       width  = 5,
       height = 5,
       useDingbats=FALSE)
...
p_atoms_OC_3132_mean_sd <- ggplot(data = atoms_OC_per_codon_in_module_df, mapping = aes(x = Carbon_mean, y = Oxygen_mean)) + # times 3 to calulate for codons
  geom_hline(yintercept = mean(integrated_df_molten_basic_atoms$Oxygen)*3, lty=2) +
  geom_vline(xintercept = mean(integrated_df_molten_basic_atoms$Carbon)*3, lty=2) +
  ...
  annotate("text", x = 13.77, y = 2.965, label = "Genome mean")+
  ...
  theme(strip.background = element_blank(),
        panel.grid.major = element_blank(),
        panel.grid.minor = element_blank())+
  ggtitle(label = expression(paste(italic("Methylobacter tundripaludum"), " 31/32")))+
  xlab(label = "Carbon atoms per codon")+
  ylab(label = "Oxygen atoms per codon")+
  NULL
ggsave(filename = "0_countergradient_mean_sd_3132.pdf",
       plot = p_atoms_OC_3132_mean_sd,
       device = "pdf",
       width  = 5,
       height = 5,
       useDingbats=FALSE)

p_delta_atoms_OC_3132_mean_sd <- ggplot(data = atoms_OC_per_codon_in_module_df, mapping = aes(x = Carbon_mean-(mean(integrated_df_molten_basic_atoms$Carbon)*3), y = Oxygen_mean-(mean(integrated_df_molten_basic_atoms$Oxygen)*3))) + # times 3 to calulate for codons
  ...
  NULL
ggsave(filename = "0_delta_countergradient_mean_sd_3132.pdf",
       plot = p_delta_atoms_OC_3132_mean_sd,
       device = "pdf",
       width  = 5,
       height = 5,
       useDingbats=FALSE)
