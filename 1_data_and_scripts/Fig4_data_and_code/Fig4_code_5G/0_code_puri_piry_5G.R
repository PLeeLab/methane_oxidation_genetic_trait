...
fasta_seqinr <- read.fasta(file = "MBUR.2-CDSs.fasta")
mage_code <- gsub(x = names(fasta_seqinr), pattern = "\\|.*", replacement = "")
rna_counts <- read.delim(file = "12934_2015_377_MOESM2_ESM.txt")
...
integrated_df <- merge(x = mage_df, y = rna_counts, by.x="mage_code", by.y="Locus_tag", sort=FALSE)
integrated_df_molten_basic <- melt(data = integrated_df, measure.vars = c(10,11,13,14))
colnames(integrated_df_molten_basic)[c(12,13)] <- c("experiment", "expression_level")
...
integrated_df_molten_basic$operon <- ifelse(test = integrated_df_molten_basic$mage_code == "MBURv2_210045" | integrated_df_molten_basic$mage_code == "MBURv2_210046" | integrated_df_molten_basic$mage_code == "MBURv2_210047", yes = "pmo",
                                            no = ifelse(test = integrated_df_molten_basic$mage_code == "MBURv2_210189", yes = "xox",
                                                        no = ifelse(test = integrated_df_molten_basic$mage_code == "MBURv2_210291" | integrated_df_molten_basic$mage_code == "MBURv2_210294", yes = "mxa",
                                                                    no = ifelse(test = integrated_df_molten_basic$mage_code == "MBURv2_130046" | integrated_df_molten_basic$mage_code == "MBURv2_130047" | integrated_df_molten_basic$mage_code == "MBURv2_130048" | integrated_df_molten_basic$mage_code == "MBURv2_130049" | integrated_df_molten_basic$mage_code == "MBURv2_130050" | integrated_df_molten_basic$mage_code == "MBURv2_130051", yes = "mmo",
                                                                                no = "other"))))
...
integrated_df_molten_basic_length <- integrated_df_molten_basic[integrated_df_molten_basic$experiment == "Batch.culture.1",]

p_puri_pyri_hist_genome_5G <- ggplot(data = integrated_df_molten_basic_length)+
  ...
  ggtitle(label = "a", subtitle = expression(paste(italic("Methylomicrobium buryatense"), " 5G")))+
...
p_pyri_exp_5G <- ggplot(data = integrated_df_molten_basic, mapping = aes(x = nPyrimidines, y = (expression_level+0.5))) +
  geom_point(alpha  = ifelse(test = integrated_df_molten_basic$operon == "other", yes = 1, no = 0),
             ...
             fill   = ifelse(test = integrated_df_molten_basic$operon == "pmo", yes = "purple", no = ifelse(test = integrated_df_molten_basic$operon == "mxa", yes = "green", no = ifelse(test = integrated_df_molten_basic$operon == "xox", yes = "yellow", no = ifelse(test = integrated_df_molten_basic$operon == "mmo", yes = "blue", no = "grey")))),
             colour = "grey",
             pch    = 21)+
  geom_point(alpha  = ifelse(test = integrated_df_molten_basic$operon == "other", yes = 0, no = 1),
             ...
             fill   = ifelse(test = integrated_df_molten_basic$operon == "pmo", yes = "#984EA3", no = ifelse(test = integrated_df_molten_basic$operon == "mxa", yes = "#4DAF4A", no = ifelse(test = integrated_df_molten_basic$operon == "xox", yes = "#FF7F00", no = ifelse(test = integrated_df_molten_basic$operon == "mmo", yes = "#377EB8", no = "grey")))),
             colour = "black",
             pch    = 21)+
  geom_vline(xintercept = 0.5, lty=2) +
  scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x), labels = trans_format("log10", math_format(10^.x)))+
  annotation_logticks(sides = "l")+
  facet_wrap(~experiment, nrow = 2, ncol = 2, scales = "free")+
  theme_classic2()+
...
colnames(integrated_df_molten)[c(12,13)] <- c("base", "count_genome")
integrated_df_molten <- melt(data = integrated_df_molten, measure.vars = c(6,7,9,10))
colnames(integrated_df_molten)[c(10,11)] <- c("experiment", "expression_level")
...
integrated_df_molten <- melt(data = integrated_df_molten, measure.vars = c(9,12,13))
colnames(integrated_df_molten)[c(11,12)] <- c("omic", "count")
...
# removing mxa
integrated_df_molten_no_mxa <- integrated_df_molten[integrated_df_molten$mage_code != "MBURv2_210291" & integrated_df_molten$mage_code != "MBURv2_210294", ]

# removing xox
integrated_df_molten_no_xox <- integrated_df_molten[integrated_df_molten$mage_code != "MBURv2_210189", ]

# removing mmo
integrated_df_molten_no_mmo <- integrated_df_molten[integrated_df_molten$mage_code != "MBURv2_130046" & integrated_df_molten$mage_code != "MBURv2_130047" & integrated_df_molten$mage_code != "MBURv2_130048" & integrated_df_molten$mage_code != "MBURv2_130049" & integrated_df_molten$mage_code != "MBURv2_130050" & integrated_df_molten$mage_code != "MBURv2_130051", ]

# removing pmo
integrated_df_molten_no_pmo <- integrated_df_molten[integrated_df_molten$mage_code != "MBURv2_210045" & integrated_df_molten$mage_code != "MBURv2_210046" & integrated_df_molten$mage_code != "MBURv2_210047", ]
...
for (u in 2:5) {t_casted[,u] <- t_casted[,u] / sum(t_casted[,u]) }; colSums(t_casted)
t_molten <- melt(data = t_casted, measure.vars = c(2:5))
t_molten$pmo <- "1T"
...
for (u in 2:5) {gtm_casted[,u] <- gtm_casted[,u] / sum(gtm_casted[,u]) }; colSums(gtm_casted)
gtm_molten <- melt(data = gtm_casted, measure.vars = c(2:5))
...
for (u in 2:5) {gtx_casted[,u] <- gtx_casted[,u] / sum(gtx_casted[,u]) }; colSums(gtx_casted)
gtx_molten <- melt(data = gtx_casted, measure.vars = c(2:5))
gtx_molten$pmo <- "3T-xox"

gts_casted <- cast(data = integrated_df_molten_no_mmo[integrated_df_molten_no_mmo$omic=="count_transcriptome", ], formula = base~experiment, value = "count", fun.aggregate = sum )
for (u in 2:5) {gts_casted[,u] <- gts_casted[,u] / sum(gts_casted[,u]) }; colSums(gts_casted)
gts_molten <- melt(data = gts_casted, measure.vars = c(2:5))
gts_molten$pmo <- "4T-mmo"
...
for (u in 2:5) {gtp_casted[,u] <- gtp_casted[,u] / sum(gtp_casted[,u]) }; colSums(gtp_casted)
gtp_molten <- melt(data = gtp_casted, measure.vars = c(2:5))
gtp_molten$pmo <- "5T-pmo"

z_dataframe <- rbind(t_molten, gtm_molten, gtx_molten, gts_molten, gtp_molten)
...
p_base_5G <- ggplot(data = z_dataframe, aes(x = pmo, y = value*100, colour=base, group=base))+
  geom_point()+
  geom_line()+
...
p_gc_5G <- ggplot(data = z_dataframe, aes(x = pmo, y = value*100, colour=gc, group=gc))+
...
p_compound_5G <- ggplot(data = z_dataframe, aes(x = pmo, y = value*100, colour=compound, group=compound))+
...
integrated_df_molten_basic_atoms <- integrated_df_molten_basic[integrated_df_molten_basic$experiment == "Batch.culture.1",] # only 1 rep is required to calculate atom content as mRNA abundance won't be accounted for

p_atoms_NC_5G <- ggplot(data = integrated_df_molten_basic_atoms, mapping = aes(x = Nitrogen*3, y = Carbon*3)) + # times 3 to calulate for codons
  ...
  geom_point(alpha  = ifelse(test = integrated_df_molten_basic_atoms$operon == "other", yes = 0, no = 1),
             ...
             fill   = ifelse(test = integrated_df_molten_basic_atoms$operon == "pmo", yes = "#984EA3", no = ifelse(test = integrated_df_molten_basic_atoms$operon == "mxa", yes = "#4DAF4A", no = ifelse(test = integrated_df_molten_basic_atoms$operon == "xox", yes = "#FF7F00", no = ifelse(test = integrated_df_molten_basic_atoms$operon == "mmo", yes = "#377EB8", no = "grey")))),             colour = "black",
             pch    = 21)+
...
p_atoms_OH_5G <- ggplot(data = integrated_df_molten_basic_atoms, mapping = aes(x = Oxygen*3, y = Hydrogen*3)) + # times 3 to calulate for codons
  geom_point(alpha  = ifelse(test = integrated_df_molten_basic_atoms$operon == "other", yes = 1, no = 0),
             size   = ifelse(test = integrated_df_molten_basic_atoms$operon == "other", yes = 2, no = 0),
             fill   = ifelse(test = integrated_df_molten_basic_atoms$operon == "pmo", yes = "purple", no = ifelse(test = integrated_df_molten_basic_atoms$operon == "mxa", yes = "green", no = ifelse(test = integrated_df_molten_basic_atoms$operon == "xox", yes = "yellow", no = "grey"))),
             colour = "grey",
...
p_Ncor <- ggplot(data = integrated_df_molten_basic_atoms, mapping = aes(x = expression_level+0.5, y = Nitrogen*3)) + # times 3 to calulate for codons
  ...
  theme(strip.background = element_blank(),
        panel.grid.major = element_blank(),
        panel.grid.minor = element_blank())+
  ggtitle(label = "i", subtitle = "Batch.culture.1")+
  xlab(label = "mRNA abundance")+
  ylab(label = "Nitrogen atoms per codon")+
  scale_x_log10(breaks = trans_breaks("log10", function(x) 10^x), labels = trans_format("log10", math_format(10^.x)))+
  annotate("text", x = 10, y = 13.3, label = paste0("UCL = ", round(Ncor$conf.int[2], digits = 2)))+
  annotate("text", x = 10, y = 13.0, label = paste0("italic(R) == ", round(Ncor$estimate[[1]], digits = 2)), parse = TRUE)+
  annotate("text", x = 10, y = 12.7, label = paste0("LCL = ", round(Ncor$conf.int[1], digits = 2)))+
...
p_Ccor <- ggplot(data = integrated_df_molten_basic_atoms, mapping = aes(x = expression_level+0.5, y = Carbon*3)) + # times 3 to calulate for codons
  ...
  ylab(label = "Carbon atoms per codon")+
  scale_x_log10(breaks = trans_breaks("log10", function(x) 10^x), labels = trans_format("log10", math_format(10^.x)))+
  annotate("text", x = 10, y = 14.30, label = paste0("UCL = ", round(Ccor$conf.int[2], digits = 2)))+
  annotate("text", x = 10, y = 14.20, label = paste0("italic(R) == ", round(Ccor$estimate[[1]], digits = 2)), parse = TRUE)+
  annotate("text", x = 10, y = 14.10, label = paste0("LCL = ", round(Ccor$conf.int[1], digits = 2)))+
  annotation_logticks(sides = "b")+
  NULL
...
p_Ocor <- ggplot(data = integrated_df_molten_basic_atoms, mapping = aes(x = expression_level+0.5, y = Oxygen*3)) + # times 3 to calulate for codons
  ...
  scale_x_log10(breaks = trans_breaks("log10", function(x) 10^x), labels = trans_format("log10", math_format(10^.x)))+
  annotate("text", x = 10, y = 3.8, label = paste0("UCL = ", round(Ocor$conf.int[2], digits = 2)))+
  annotate("text", x = 10, y = 3.7, label = paste0("italic(R) == ", round(Ocor$estimate[[1]], digits = 2)), parse = TRUE)+
  annotate("text", x = 10, y = 3.6, label = paste0("LCL = ", round(Ocor$conf.int[1], digits = 2)))+
  annotation_logticks(sides = "b")+
  NULL
...
p_Hcor <- ggplot(data = integrated_df_molten_basic_atoms, mapping = aes(x = expression_level+0.5, y = Hydrogen*3)) + # times 3 to calulate for codons
  ...
  geom_point(alpha  = ifelse(test = integrated_df_molten_basic_atoms$operon == "other", yes = 0, no = 1),
             size   = ifelse(test = integrated_df_molten_basic_atoms$operon == "other", yes = 0, no = 3),
             fill   = ifelse(test = integrated_df_molten_basic_atoms$operon == "pmo", yes = "#984EA3", no = ifelse(test = integrated_df_molten_basic_atoms$operon == "mxa", yes = "#4DAF4A", no = ifelse(test = integrated_df_molten_basic_atoms$operon == "xox", yes = "#FF7F00", no = ifelse(test = integrated_df_molten_basic_atoms$operon == "mmo", yes = "#377EB8", no = "grey")))),
             colour = "black",
             pch    = 21)+
  geom_smooth(method = "lm", colour="black", lty=2)+
  theme_classic2()+
  ...
  ylab(label = "Hydrogen atoms per codon")+
  scale_x_log10(breaks = trans_breaks("log10", function(x) 10^x), labels = trans_format("log10", math_format(10^.x)))+
  annotate("text", x = 5, y = 14.70, label = paste0("UCL = ", round(Hcor$conf.int[2], digits = 2)))+
  annotate("text", x = 5, y = 14.65, label = paste0("italic(R) == ", round(Hcor$estimate[[1]], digits = 2)), parse = TRUE)+
  annotate("text", x = 5, y = 14.60, label = paste0("LCL = ", round(Hcor$conf.int[1], digits = 2)))+
  annotation_logticks(sides = "b")+
  NULL
...
lay_matrix <- rbind(c(1, 2, NA),
                    ...
                    c(13, 13, 13),
                    c(13, 13, 13))
ggsave(filename = "0_puriPiry_5G.pdf",
       plot   = grid.arrange(p_puri_pyri_hist_genome_5G, p_pyri_exp_5G, p_base_5G, p_gc_5G, p_compound_5G, p_atoms_NC_5G, p_atoms_OH_5G, p_stats_Pyri, p_stats_C, p_stats_H, p_stats_O, p_stats_N, p_cors, layout_matrix = lay_matrix),
       device = "pdf",
       width  = 15,
       height = 25,
       useDingbats=FALSE)
...
atoms_C_per_codon_in_module_df <- rbind(atoms_C_per_codon_in_module_df,
                                        data.frame(operon = "mmo",
                                                   Mean = mean(integrated_df_molten_basic_atoms_only_module$Carbon[integrated_df_molten_basic_atoms_only_module$operon == "mmo"]),
                                                   SD = sd(integrated_df_molten_basic_atoms_only_module$Carbon[integrated_df_molten_basic_atoms_only_module$operon == "mmo"])))
...
atoms_O_per_codon_in_module_df <- rbind(atoms_O_per_codon_in_module_df,
                                        data.frame(operon2 = "mmo",
                                                   Mean = mean(integrated_df_molten_basic_atoms_only_module$Oxygen[integrated_df_molten_basic_atoms_only_module$operon == "mmo"]),
                                                   SD = sd(integrated_df_molten_basic_atoms_only_module$Oxygen[integrated_df_molten_basic_atoms_only_module$operon == "mmo"])))
...
p_atoms_OC_5G_mean_sd <- ggplot(data = atoms_OC_per_codon_in_module_df, mapping = aes(x = Carbon_mean, y = Oxygen_mean)) + # times 3 to calulate for codons
  ...
  annotate("text", x = 13.72, y = 2.965, label = "Genome mean")+
  geom_point(fill   = c("#984EA3", "#4DAF4A", "#FF7F00", "#377EB8"),
             colour = "black",
             pch    = 21,
             size = 5)+
  theme_linedraw()+
  theme(strip.background = element_blank(),
        panel.grid.major = element_blank(),
        panel.grid.minor = element_blank())+
  ggtitle(label = expression(paste(italic("Methylomicrobium buryatense"), " 5G")))+
  xlab(label = "Carbon atoms per codon")+
  ylab(label = "Oxygen atoms per codon")+
  NULL
ggsave(filename = "0_countergradient_mean_sd_5G.pdf",
       plot = p_atoms_OC_5G_mean_sd,
       device = "pdf",
       width  = 5,
       height = 5,
       useDingbats=FALSE)

p_delta_atoms_OC_5G_mean_sd <- ggplot(data = atoms_OC_per_codon_in_module_df, mapping = aes(x = Carbon_mean-(mean(integrated_df_molten_basic_atoms$Carbon)*3), y = Oxygen_mean-(mean(integrated_df_molten_basic_atoms$Oxygen)*3))) + # times 3 to calulate for codons
  ...
  NULL
ggsave(filename = "0_delta_countergradient_mean_sd_5G.pdf",
       plot = p_delta_atoms_OC_5G_mean_sd,
       device = "pdf",
       width  = 5,
       height = 5,
       useDingbats=FALSE)
