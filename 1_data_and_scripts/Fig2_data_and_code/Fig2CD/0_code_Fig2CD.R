...
dim(genes_metadata)[1] == dim(Nc_all_df)[1] # Must be TRUE
integrated_Nc_df <- merge(x = genes_metadata, y = Nc_all_df, by="CDS", all.x = TRUE)
length(Nc_all_df$CDS)==dim(integrated_Nc_df)[1] # Must be FALSE
...
dim(Nc_all_df)[1]==dim(integrated_Nc_df)[1] # Must be TRUE
...
p_Fig2DS1 <- ggplot(integrated_Nc_df, aes(x=GC3, y=Nc)) +
  ...
  guides(fill = guide_legend(title="Gene", override.aes=list(shape=21)), shape = guide_legend(title="", override.aes=list(color="black")))+
  NULL
ggsave(filename = "Fig2DS1.pdf", plot = p_Fig2DS1, device = "pdf", width = 9, height = 8, useDingbats=FALSE)
